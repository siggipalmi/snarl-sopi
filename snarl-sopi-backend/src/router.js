...
function handleListProducts(req, res) {
  const products = storage.listProducts().map(p => ({
    goodsId: p.goodsId, customCode: p.customCode, name: p.name,
...
async function handleCreateProduct(req, res) {
  ...
  const vatRate = (Number(b.vatRate) === 11) ? 11 : 24;            // VSK 11 or 24 (default 24)
  ...
  // Store our own attributes (weight / VSK / cost) keyed by the Weimi goodsId
  // (which matches orders.goodsId, so the sales report can join on it).
  try {
    storage.upsertProduct({
      goodsId, customCode, name: fields.goodsName,
  ...
  ok(res, { ok: true, goodsId, customCode, imgUrl, goodsName: fields.goodsName,
