...
function handleFridgeComplaint(req, res) {
  ...
  if (!m) return contractError(res, 404, 'device_not_found', `Vélnúmer ${deviceCode} er ekki skráð.`, `Device ${deviceCode} is not registered.`);
  const c = req.body || {};
  const REASONS = ['not_taken', 'wrong_quantity', 'returned_but_charged', 'other'];
  const errors = [];
  ...
  // Reuse the coil pattern-alert if 3+ in 24h (a basket miscalibration shows up as a cluster).
  try {
...
const CMD_TYPES = ['clear_aisle_fault', 'set_drop_sensor', 'query_channel_status', 'restart_app', 'restart_machine', 'set_temp', 'set_cooling', 'defrost', 'fridge_open_door', 'scale_read', 'scale_calibrate', 'scale_tare'];
...
async function handlePublishAppRelease(req, res) {
  ...
  // Forward-only guard: a new release must have a strictly higher targetVersionCode than the one
  // currently published for this app. This catches the "APK updated but version left stale" class
  // of mistake at publish time — a manifest whose version disagrees with its artifact would
  // silently disable the client's rollback protection, so refuse rather than store it.
  ...
  // Echo back exactly what was stored (re-read), so a caller sees the persisted values, not just
  // what they sent — makes any store/round-trip discrepancy immediately visible.
  const stored = storage.getAppRelease(appKey);
  ok(res, { release: stored, note });
}
