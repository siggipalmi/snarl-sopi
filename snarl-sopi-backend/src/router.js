...
async function handlePublishAppRelease(req, res) {
  ...
  const selfApkUrl = ((process.env.APP_URL || 'https://admin.agvending.is').replace(/\/$/, '')) + '/api/v1/app-release/' + appKey + '/apk';

  const rel = {
    targetVersionCode: tvc,
    versionName: b.versionName ? String(b.versionName) : null,
    apkUrl: selfApkUrl,            // machine downloads from us, not the origin
