...
// name (retail). Returns { matchedGoodsId, matchStatus, candidates }.
function matchLine(line) {
  const storage = require('./storage');
  // Barcode exact-ish (prefix search then exact filter).
  if (line.barcode) {
  ...
  // Name (truncation-tolerant: search returns LIKE %term%).
  const term = String(line.description || '').replace(/\d+x\d+.*$/i, '').trim(); // drop trailing pack spec
  const byName = term.length >= 3 ? storage.searchProducts(term, 6) : [];
  if (byName.length === 1) return { matchedGoodsId: byName[0].goodsId, matchStatus: 'auto', candidates: byName };
  return { matchedGoodsId: null, matchStatus: 'unmatched', candidates: byName };
}
...
function handleConfirmReceipt(req, res) {
  ...
  // Validate: every stored line must be resolved (matched) or skipped.
  ...
  let written;
  try { written = storage.confirmReceiptWrites(r.id, writes); }
  catch (e) { return serverError(res, new Error('confirm failed: ' + e.message)); }

  ok(res, { confirmed: true, written, count: written.length });
}
