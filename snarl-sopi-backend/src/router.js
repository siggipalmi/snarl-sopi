 ...
 * Change prices. Weimi's price endpoint is WHOLE-MACHINE (every slot must be
 * resubmitted), so the backend reads the machine's current config fresh, swaps
 * in only the changed prices, and resubmits everything — untouched slots keep
 * their exact current values. body: { changes: [{ aisleCode, priceIsk }] }.
 */
async function handleSlotPrice(req, res) {
  ...
  // aisleCode → new price in CENTS (Weimi stores price ×100)
  const priceMap = {};
  ...
  if (!Object.keys(priceMap).length) return json(res, 400, { ok: false, error: 'no valid price changes' });

  // Read the machine's current config fresh (whole machine must be resubmitted).
  let info;
  ...
  const configured = aisles.filter(a => a.code && (a.goodsId || a.id));
  if (!configured.length) return json(res, 400, { ok: false, error: 'no configured slots on this machine' });

  // Rebuild the whole-machine list from CURRENT values; override only target prices.
  let applied = 0;
  const aisleList = configured.map(a => {
    const item = {
      aisleCode:   a.code,
    ...
    if (Number(a.measurement) === 1 && a.goodsWeight != null) item.goodsWeight = a.goodsWeight;
    if (priceMap[a.code] != null) { item.price = priceMap[a.code]; applied++; }
    return item;
  });
  if (!applied) return json(res, 400, { ok: false, error: 'none of those slots were found on this machine' });

  let result;
  try { result = await weimi.updateAisleGoodsInfo({ endpoint: 'prod' }, code, aisleList); }
  catch (e) { return json(res, 502, { ok: false, error: 'weimi_unreachable', message: e.message }); }
  ...
  ok(res, { deviceCode: code, updated: applied, totalSlots: aisleList.length, operationStatus: verdict.operationStatus });
}
