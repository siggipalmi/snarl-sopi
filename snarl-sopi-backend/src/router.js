...
function handleAddMachine(req, res) {
  ...
  machines[deviceCode] = {
    ...
    settings: { showAdRegion: true, showLeftHero: true, showRightHero: true, showIdleScreen: false, idleTimeoutSeconds: 60, defaultLanguage: 'Icelandic', availableLanguages: ['Icelandic', 'English'], hasHeatedGlass: true, heatedGlassDefaultOn: true, hasLedStrips: true, motorSerialPort: '/dev/ttyS3', controlBoardAddress: 0 },
...
function handleUpdateMachine(req, res) {
  ...
  if (req.body.model !== undefined && String(req.body.model).trim()) {
    const wasFridge = fridgeSpec(m.model).isFridge;
    m.model = String(req.body.model).trim();
    const spec = fridgeSpec(m.model);
    m.isKioskModel = !spec.isFridge;
    // Changing the kind must also correct stockSource, or a machine mislabelled as coil keeps the
    // coil default ('kiosk') after being corrected to a fridge — and then runs the wrong stock path
    // while looking fixed. Fridges weigh their own stock, coil machines are kiosk-owned. Skip if the
    // caller set stockSource explicitly in the same request.
    if (spec.isFridge !== wasFridge && req.body.stockSource === undefined) {
      m.stockSource = spec.isFridge ? 'weimi' : 'kiosk';
      console.log(`[MACHINE] ${m.deviceCode} kind changed to ${spec.isFridge ? 'fridge' : 'coil'} — stockSource set to ${m.stockSource}`);
    }
  }
...
// POST /machines/:deviceCode/ads/upload — host an ad poster on R2 and return its URL.
// The kiosk ad region is HD landscape (1920x1080, 16:9). We accept anything and report what the
// image actually is rather than silently cropping: a poster that arrives at the wrong ratio will be
// letterboxed or cut on the machine, and finding that out from the dashboard beats finding out from
// a customer looking at a stretched logo.
async function handleUploadAd(req, res) {
  ...
  const b = req.body || {};
  if (!b.data || typeof b.data !== 'string') return badRequest(res, 'data (base64 image) is required');
  let contentType = 'image/jpeg', data = b.data;
  const mm = /^data:([^;]+);base64,(.*)$/s.exec(data);
  if (mm) { contentType = mm[1]; data = mm[2]; }
  const buf = Buffer.from(data, 'base64');
  if (!buf.length) return badRequest(res, 'Image is empty');
  if (buf.length > 8 * 1024 * 1024) return badRequest(res, 'Image too large (max 8MB)');

  let width = null, height = null, note = null, out = buf, outType = contentType;
  try {
    ...
    if (width && height) {
      ...
      const off = Math.abs(ratio - 16 / 9);
      if (off > 0.02) {
        note = `This image is ${width}x${height} (${ratio.toFixed(2)}:1). The kiosk ad region is 16:9, so it will be cropped or letterboxed. 1920x1080 fits exactly.`;
      } else if (width < 1280) {
        note = `This image is ${width}x${height}. It is the right shape but will look soft on a 1080p screen — 1920x1080 is the target.`;
      }
      // Re-encode oversized posters down to 1920 wide; anything larger is bytes the kiosk has to
      // pull over a machine's connection for no visible gain.
      if (width > 1920) {
        out = await sharp(buf).resize(1920, null, { withoutEnlargement: true }).jpeg({ quality: 86 }).toBuffer();
        outType = 'image/jpeg';
        note = (note ? note + ' ' : '') + `Resized from ${width}px wide to 1920px.`;
      }
    }
...
function handleEnqueueCommand(req, res) {
  ...
  if (type === 'clear_aisle_fault') {
  ...
  } else if (type === 'tare_all') {
    // force skips the per-basket empty check. Taring a LOADED basket sets a false zero and
    // mis-charges every later sale from it, so this is only correct on a machine that is actually
    // empty — the dashboard asks that question rather than asking about "force".
    ...
    if (!/^\/dev\/ttyS\d+$/.test(port)) {
      return badRequest(res, `port must look like /dev/ttyS1 — got "${port}"`);
    }
    // The app refuses /dev/ttyS3 on gravity machines because that is the weight bus. Rather than
    // hardcode that, check against THIS machine's own configured bus port: pointing the payment link
    // at the bus a machine already uses would open a second reader on it and corrupt both at once.
    // A hardcoded rule would also be wrong on coil, where ttyS3 is the motor bus, not a free port.
    const mp = machines[deviceCode];
    const busPort = (mp && mp.settings && mp.settings.motorSerialPort) ? String(mp.settings.motorSerialPort).trim() : null;
    ...
    // guarded so it cannot be satisfied by a careless paste. The caller must name the machine as
    // well as confirm, mirroring the set_machine_key guard.
    if (params.confirm !== 'yes') {
      return badRequest(res, 'clear_device_owner requires params.confirm = "yes". This ends silent OTA for this machine permanently — every later update will need someone on site with adb.');
    }
    if (params.deviceCode !== deviceCode) {
      return badRequest(res, `clear_device_owner requires params.deviceCode to match the machine (${deviceCode}). Naming it is deliberate: this strands the machine you name, and it is not undoable remotely.`);
    }
    // Leave a record. If this is ever issued by mistake, the operator should hear about it now
    // rather than discover it at the next update.
    try {
      const mach = machines[deviceCode];
      const detail = `${deviceCode} was told to surrender Device Owner. Silent over-the-air updates are no longer possible on this machine — every future build needs someone on site with adb. If this was not intended, the machine must be re-provisioned in person.`;
      storage.insertAlert({
        id: 'alert_device_owner_cleared_' + deviceCode + '_' + Date.now(),
        type: 'device_owner_cleared', severity: 'critical',
        title: `Device Owner surrendered — ${(mach && mach.deviceName) || deviceCode}`,
      ...
      if (mach) {
        ...
        if (op && toEmail) {
          email.sendOperatorAlert({
            to: toEmail, operatorName: op.name,
            title: `Device Owner surrendered — ${mach.deviceName || deviceCode}`,
            detail,
            dashboardUrl: (process.env.APP_URL || 'https://admin.agvending.is') + '/?page=machines&code=' + deviceCode,
          }).catch(err => console.error('[DO] email failed:', err && err.message));
        }
      }
      console.warn('[DO] ' + detail);
    } catch (e) { /* never block the command on alerting */ }
  } else if (type === 'read_all_trays' || type === 'read_temp' || type === 'launch_support') {
    // read-only, no params
...
function handleCommandResult(req, res) {
  ...
  ['aisle', 'rawSlot', 'outcome', 'resultCode', 'resultCodeHex', 'boardDetail', 'orderId',
   'closedAisles', 'undecodableAisles', 'columnBaseZero', 'stockSource', 'dropSensorDesiredOn', 'keySource', 'trays', 'addresses', 'tared', 'refused', 'noReply', 'tempC',
  ...
  // 'unsupported' means the dashboard offered a control this machine's build can't perform. The
  // operator saw a "sent" toast and nothing happened — the button was dead and nothing said so.
  // That has now bitten three times (clear_aisle_fault no-opping, read_temp, set_temp vs
  // set_cooling), so surface it instead of storing it quietly.
  if (finalStatus === 'unsupported') {
    try {
      const mach = machines[deviceCode];
      const ver = (mach && mach.kioskVersion) ? ('app v' + mach.kioskVersion) : 'this build';
      const detail = `${deviceCode} · the "${cmd.type}" control was sent but ${ver} does not handle it, so nothing happened on the machine. ` +
        `Either the machine needs updating, or that control should be hidden for this build.`;
      const alertId = 'alert_unsupported_cmd_' + deviceCode + '_' + cmd.type + '_' + Math.floor(Date.now() / (24 * 3600 * 1000));
      if (!storage.getAlert(alertId)) storage.insertAlert({
        id: alertId, type: 'unsupported_command', severity: 'warning',
        title: `Control does nothing — ${(mach && mach.deviceName) || deviceCode}`,
        detail, deviceCode, resolved: false, createdAt: new Date().toISOString(),
      });
...
function handleUpdateSettings(req, res) {
  ...
  const allowed = ['showAdRegion','showLeftHero','showRightHero','showIdleScreen','idleTimeoutSeconds','defaultLanguage','availableLanguages','hasHeatedGlass','heatedGlassDefaultOn','hasLedStrips','motorSerialPort','controlBoardAddress',
  ...
  if (req.body.paymentSerialPort !== undefined) {
    ...
    const busPort = (m.settings && m.settings.motorSerialPort) ? String(m.settings.motorSerialPort).trim() : null;
