...
async function handleUpdateProduct(req, res) {
  ...
  let newImgUrl = null, imageHasBackground = null, clearedPct = null;
  if (b.imageBase64) {
    ...
    try {
      ...
      newImgUrl = norm.url; imageHasBackground = norm.hasBackground; clearedPct = norm.clearedPct;
  ...
  if (newImgUrl) {
    try {
      if (!existing.goodsId) storage.ensureProductStub(goodsId, b.goodsName || null);
      storage.setProductImage(goodsId, {
        imgUrl: newImgUrl, hasBackground: imageHasBackground, clearedPct,
  ...
  if (!current || !current.goodsId) {
    ...
    try {
      ...
      if (newImgUrl) storage.setProductImage(goodsId, { imgUrl: newImgUrl, hasBackground: imageHasBackground, clearedPct, weimiImgUrl: null });
