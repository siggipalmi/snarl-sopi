...
function handleFridgeComplaint(req, res) {
  ...
  notifyOperatorOfFridgeComplaint(
    { id, deviceCode, orderId: String(c.orderId), reason: c.reason,
      ...
      timestampMs: c.timestampMs },
    m
  ).catch(err => console.error('[FRIDGE-COMPLAINT] notify failed:', err && err.message));
  created(res, { complaintId: id });
}
...
// Who gets complaint mail for this machine: the operator's contactEmail, else an operator_admin
// user, else AG Vending. Shared by the coil and fridge complaint notifications.
function resolveOperatorEmail(machine) {
  ...
  if (!toEmail) {
    ...
    toEmail = (opUsers.find(u => u.role === 'operator_admin') || {}).email || null;
  }
  if (!toEmail) {
...
async function notifyOperatorOfFridgeComplaint(complaint, machine) {
  ...
  return email.sendFridgeComplaintToOperator({
    ...
    deviceCode: machine.deviceCode, complaint, temp, dashboardUrl,
  });
}
...
const CMD_TYPES = ['clear_aisle_fault', 'set_aisle_enabled', 'set_drop_sensor', 'query_channel_status', 'restart_app', 'restart_machine', 'set_temp', 'set_cooling', 'defrost', 'fridge_open_door', 'scale_read', 'scale_calibrate', 'scale_tare', 'check_update'];
