...
async function handleSaveGoodsTest(req, res) {
  ...
  const fields = {
    ...
    measurement: 0,
    remark: 'created by snarl-sopi save-goods test',
  };
  try {
    const result = await weimi.saveGoodsRaw({ endpoint: 'prod' }, fields);
    ok(res, { ok: true, imgUrl, sent: fields, weimi: result, note: 'If weimi.weimiCode is 200, the product was created. Delete it in the portal.' });
  } catch (e) {
