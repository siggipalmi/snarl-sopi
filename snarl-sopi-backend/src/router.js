...
async function handleMigrateImages(req, res) {
  ...
  for (const c of todo) {
    try {
      ...
      storage.setProductImage(c.goodsId, { imgUrl: r.url, hasBackground: r.hasBackground, weimiImgUrl: c.src });
      report.migrated++;
...
async function handleCleanBackgrounds(req, res) {
  ...
  for (const p of targets) {
    ...
    try {
      ...
      const url = await r2.putObject(`products/${safeId}-${Date.now()}.webp`, norm.buffer, 'image/webp');
      storage.setProductImage(p.goodsId, { imgUrl: url, hasBackground: norm.hasBackground, weimiImgUrl: src, clearedPct: norm.clearedPct });
      ...
      if (!norm.hasBackground) {
        ...
        // Knockout couldn't help: the backdrop isn't white (photo/gradient/shadowbox).
