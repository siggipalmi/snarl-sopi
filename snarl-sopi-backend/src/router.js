...
function handleFridgeSettlement(req, res) {
  ...
  // Planogram lookup for recompute: (cabinet, basket) → { unitWeightG, priceIsk }.
  const plan = {};
  ...
  const lineRows = lines.map(l => {
    ...
    const pl = plan[key] || {};
    // Recompute from weights + planogram. Prefer the planogram's unit weight/price over the
    // app's, so a stale or wrong app value shows up as a mismatch rather than being trusted.
    ...
    return {
      orderId: b.orderId, cabinet: l.cabinet || null, basket: l.basket != null ? Math.round(l.basket) : null,
  ...
  const isRepost = !!storage.getFridgeSettlement(b.orderId);
...
function handleGetFridgeBaskets(req, res) {
  ...
  const spec = require('./db').fridgeSpec ? require('./db').fridgeSpec(m.model) : null;
...
// Upserts each; a basket with productId null is treated as empty/disabled.
...
function handleListFridgeSettlements(req, res) {
  ...
  const rows = storage.listFridgeSettlements(deviceCode, Number(req.query.limit) || 100).map(s => ({
    ...s,
    cabinetsOpened: safeJsonParse(s.cabinetsOpened, []),
    anomalies: safeJsonParse(s.anomalies, []),
  }));
...
function downloadsKeyOk(req) {
  ...
  if (!want) return false;                      // unset = page disabled (fail closed)
...
<title>Downloads · Snarl &amp; Sopi</title>
<style>
  ...
  *{box-sizing:border-box}
  ...
  .pkg{display:flex;align-items:center;justify-content:space-between;gap:12px;background:#FFFFFFcc;border:.5px solid var(--line);border-radius:14px;padding:15px 16px;margin-bottom:9px;text-decoration:none;color:inherit;transition:transform .06s ease,box-shadow .12s ease;}
  ...
  <footer>Snarl &amp; Sopi · AG Vending</footer>
</div></body></html>`;
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache' });
  res.end(html);
}

// GET /api/v1/downloads — registry as JSON (ag-admin, for the management UI).
function handleGetDownloads(req, res) { ok(res, { downloads: storage.listDownloads() }); }

// PUT /api/v1/downloads — replace the whole registry. Body: { downloads: [ {app, versionName,
// versionCode, url, notes} ] }. Simple whole-list replace keeps the admin UI trivial.
