...
function handleAppUpdate(req, res) {
  ...
  // Resolve the (relative) apkUrl to an ABSOLUTE URL on the SAME origin this request came in on,
  // so the machine downloads the APK from the exact host it just talked to — never a different
  // hostname with its own cert chain. Falls back gracefully if the path is already absolute
  // (legacy releases) or the host header is missing.
  let apkUrl = rel.apkUrl || null;
  if (apkUrl && apkUrl.startsWith('/')) {
    const host = req.headers['x-forwarded-host'] || req.headers.host;
    const proto = (req.headers['x-forwarded-proto'] || 'https').split(',')[0].trim();
    if (host) apkUrl = proto + '://' + host + apkUrl;
  }
  json(res, 200, {
