...
function handleHealth(req, res) {
  ok(res, { status: 'ok', version: '1.1.0', contract: 'v0.1', uptime: process.uptime() });
}
...
async function handleWeimiDevices(req, res) {
  try {
    const codes    = Object.keys(machines);
    const profiles = await weimi.deviceProfile(apiConfig, codes);
    profiles.forEach(p => {
    ...
    ok(res, profiles, { synced: profiles.length });
  } catch (err) {
    console.error('[Weimi] deviceProfile:', err.message);
    json(res, 502, { ok: false, error: 'Weimi API error', detail: err.message });
  }
}
...
async function handleWeimiDevice(req, res) {
  const { deviceCode } = req.params;
  try {
    const info      = await weimi.deviceInfo(apiConfig, deviceCode);
    ...
    ok(res, { deviceCode, productCount: products.length, products });
  } catch (err) {
...
async function handleWeimiOrders(req, res) {
  ...
  try {
    const records = await weimi.queryOrders(apiConfig, { page: parseInt(page)||1, size: parseInt(size)||50, deviceCode, startDate, endDate });
    ok(res, records.map(o => ({ ...o, statusLabel: {1:'success',2:'failed',3:'refunded'}[o.status]||'unknown', amountKr: Math.round((o.totalAmount||0)/100), machineName: machines[o.deviceCode]?.deviceName || o.deviceCode })));
  } catch (err) {
...
async function handleWeimiSync(req, res) {
  ...
  try {
    const [profiles, info] = await Promise.all([
      weimi.deviceProfile(apiConfig, [deviceCode]),
      weimi.deviceInfo(apiConfig, deviceCode),
    ]);
    ...
    m.updatedAt      = new Date().toISOString();
    ok(res, { deviceCode, deviceName: m.deviceName, isOnline: m.isOnline, productCount: products.length, totalStock: m.totalCurrStock, syncedAt: m.updatedAt });
  } catch (err) {
