...
async function handleCreateProduct(req, res) {
  ...
  const fields = {
    ...
    imgUrl, thumbnailUrl: imgUrl,
    measurement: Number(b.measurement) === 1 ? 1 : 0,
  };
  ...
  ok(res, { ok: true, goodsId, customCode, imgUrl, priceIsk: Math.round(isk), goodsName: fields.goodsName });
}
...
async function handleOrderTimes(req, res) {
  ...
  try {
    ...
    const sample = (list || []).slice(0, 12).map(o => {
      ...
      const parsedUTC = raw ? Date.parse(String(raw).replace(' ', 'T') + 'Z') : null;
      return {
        ...
        parsedAsUTC_ISO: parsedUTC ? new Date(parsedUTC).toISOString() : null,
    ...
    ok(res, {
      ...
      note: 'Compare rawTimeString to the actual local time of a recent sale. Iceland = UTC. If parsedAsUTC_ISO differs from the true time, Weimi times are in another zone and the parse must be corrected.',
...
function handleProductCatalog(req, res) {
  ...
  const products = Object.values(byGoods).map(g => {
    ...
    return { goodsId: g.goodsId, name: g.name || ('#' + g.goodsId), image: g.image || '', priceIsk, machineCount: g.machines.size, slotCount: g.slotCount };
