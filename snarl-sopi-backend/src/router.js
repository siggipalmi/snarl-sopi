...
async function normalizeAndHost(buf, goodsId, opts = {}) {
  ...
  return { url, hasBackground: norm.hasBackground, note: norm.note, bytes: norm.bytes, knockedOut: norm.knockedOut };
}
...
async function handleProductImage(req, res) {
  ...
  try {
    ...
    storage.setProductImage(gid, { imgUrl: r.url, hasBackground: r.hasBackground, weimiImgUrl: p.imageNormalizedAt ? null : (p.imgUrl || null) });
...
async function handleMigrateImages(req, res) {
  ...
  const todo = all.filter(p => p.imgUrl && /^https?:\/\//.test(p.imgUrl) && (force || !p.imageNormalizedAt)).slice(0, limit);
  const report = { total: all.length, candidates: todo.length, migrated: 0, skipped: 0, failed: 0, notes: [] };
  for (const p of todo) {
    try {
      const ctrl = new AbortController();
      ...
      let buf;
      try {
        const resp = await fetch(p.imgUrl, { redirect: 'follow', signal: ctrl.signal });
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        buf = Buffer.from(await resp.arrayBuffer());
      } finally { clearTimeout(to); }
      const r = await normalizeAndHost(buf, p.goodsId, {});
      storage.setProductImage(p.goodsId, { imgUrl: r.url, hasBackground: r.hasBackground, weimiImgUrl: p.imgUrl });
      report.migrated++;
      if (r.note) report.notes.push({ goodsId: p.goodsId, name: p.name, note: r.note });
    } catch (e) {
      report.failed++;
...
async function handleUpdateProduct(req, res) {
  ...
  let imageHasBackground = null;   // set when we (re)host an image below
  if (b.imageBase64) {
    ...
    // Normalize before hosting: square-padded 800x800 WebP, alpha preserved, background
    // mode auto-detected. Keeps every tile consistent regardless of the source shot.
    try {
      const norm = await normalizeAndHost(buf, current.goodsCustomCode || goodsId, { knockoutWhite: !!b.knockoutWhite });
      imgUrl = norm.url; thumbnailUrl = norm.url; imageHasBackground = norm.hasBackground;
  ...
  try {
    ...
    // guard exists to stop Weimi sync clobbering us). An operator upload IS authoritative,
    // so set it explicitly here along with the detected background mode.
    if (imageHasBackground !== null) {
      storage.setProductImage(goodsId, {
        imgUrl, hasBackground: imageHasBackground,
        weimiImgUrl: existing.imageNormalizedAt ? null : (existing.imgUrl || null),
      });
    }
  } catch (e) { console.error('[products] upsert failed:', e.message); }

  ok(res, { ok: true, goodsId, goodsName, imgUrl, imageHasBackground, vatRate, costPriceIsk, weightGrams, measurement, barcode: barcode || null });
}
