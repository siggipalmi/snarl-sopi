...
function handleMachineRestart(req, res) {
  ...
  touchConfig(m);
  ok(res, { deviceCode: code, target, issuedAt: now, configVersion: m.configVersion });
}
...
function handleMatchDetails(req, res) {
  ...
  const catalog = storage.listProducts().map(p => ({
    goodsId: p.goodsId, name: p.name || '', tokens: pdTokens(p.name), existing: pdExisting(p),
  }));
  const results = rows.map((r, idx) => {
    const sTok = pdTokens(r && r.name);
    const scored = catalog.map(c => ({
      goodsId: c.goodsId, name: c.name, score: Math.round(pdScore(sTok, c.tokens) * 100) / 100, existing: c.existing,
 ...
 * Body: { items: [{ goodsId, details:{...} }] }  — details already conflict-resolved
 * by the operator in the review screen. Writes only the detail columns.
 */
function handleApplyDetails(req, res) {
  ...
  for (const it of items) {
    if (!it || !it.goodsId || !storage.getProduct(it.goodsId)) continue;
