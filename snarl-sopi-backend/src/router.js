...
const CMD_TYPES = ['clear_aisle_fault', 'set_drop_sensor', 'query_channel_status'];
...
function handleProductSearch(req, res) {
  ...
  const rows = storage.searchProducts(q, 10).map(p => ({
    goodsId: p.goodsId, name: p.name, barcode: p.barcode || null,
    imgUrl: p.imgUrl || layoutImg(String(p.goodsId)) || null,
    salePriceIsk: p.salePriceIsk != null ? p.salePriceIsk : layoutPrice(String(p.goodsId)),
    stock: storage.stockForProduct(p.goodsId, machines),
  }));
