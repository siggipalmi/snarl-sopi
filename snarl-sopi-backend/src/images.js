...
async function normalizeProductImage(input, opts = {}) {
  ...
  if (opts.knockoutWhite) {
    try {
      const ko = await knockoutWhiteBackground(working);
      // Guard: if it ate almost everything, the "background" wasn't background.
      if (ko.clearedPixels / ko.totalPixels > 0.92) {
  ...
  return { buffer, contentType: 'image/webp', hasBackground, size, bytes: buffer.length, knockedOut, note };
}
