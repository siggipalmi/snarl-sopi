...
async function normalizeProductImage(input, opts = {}) {
  ...
  // Record the SOURCE dimensions — after normalization everything is 800x800, so the only
  // way to know an image was a tiny upscaled source (grainy on the machine) is to capture
  // how big it was here, before we pad it.
  let srcW = null, srcH = null;
  try { const sm = await sharp(working, { failOn: 'none' }).metadata(); srcW = sm.width || null; srcH = sm.height || null; }
  catch (e) { /* leave null */ }
  ...
  return { buffer, contentType: 'image/webp', hasBackground, size, bytes: buffer.length, knockedOut, clearedPct, uniform, bgColor, srcW, srcH, note };
}
