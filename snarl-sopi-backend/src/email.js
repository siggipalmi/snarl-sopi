...
async function sendFridgeComplaintToOperator({ to, operatorName, machineName, deviceCode, complaint, temp, tempNote, dashboardUrl }) {
  const REASON_IS = {
  ...
  let tempText = tempNote || 'Engin hitamæling til staðar.';
  let tempHtml = '<div style="color:#8A8275">' + escapeHtml(tempNote || 'Engin hitamæling til staðar.') + '</div>';
...
// The daily machine digest: one email per opted-in machine, sent in the morning. Answers "is this
// machine alright?" in one glance — status first, then what needs restocking, then anything wrong.
// A machine with nothing to report sends nothing.
async function sendMachineDigest({ to, operatorName, machineName, deviceCode, status, lowStock, alerts, dashboardUrl }) {
  const out = lowStock.filter(i => i.soldOut);
  const low = lowStock.filter(i => !i.soldOut);
  const headline = out.length
    ? `${out.length} sold out, ${low.length} low`
    : (low.length ? `${low.length} item${low.length === 1 ? '' : 's'} low` : 'all stocked');
  const subject = `${machineName} — ${headline}`;

  const statusBits = [];
  statusBits.push(status.online ? 'Nettengd' : `ÓTENGD${status.lastSeenText ? ' — ' + status.lastSeenText : ''}`);
  if (status.tempC != null) statusBits.push(`${status.tempC}°C${status.tempOver ? ' — YFIR MÖRKUM' : ''}`);
  if (status.appVersion) statusBits.push(`app v${status.appVersion}`);
  if (status.todayIsk != null) statusBits.push(`${status.todayIsk.toLocaleString('is-IS')} kr í dag`);

  const listText = (arr) => arr.map(i =>
    `  • ${i.name} — ${i.stock} eftir${i.where ? ` (hólf ${i.where})` : ''}`).join('\n');

  const text = `${machineName} (${deviceCode})
${statusBits.join(' · ')}

${out.length ? `UPPSELT (${out.length}):\n${listText(out)}\n\n` : ''}${low.length ? `AÐ KLÁRAST (${low.length}):\n${listText(low)}\n\n` : ''}${alerts.length ? `ATHUGA (${alerts.length}):\n${alerts.map(a => `  • ${a.title}`).join('\n')}\n\n` : ''}${dashboardUrl}

— ${OPERATOR_BRAND}`;

  const row = (i, isOut) => `<tr>
    <td style="padding:7px 0;border-bottom:.5px solid #E8E2D8">${escapeHtml(i.name)}${i.where ? ` <span style="color:#8A8275;font-family:monospace;font-size:11px">hólf ${escapeHtml(String(i.where))}</span>` : ''}</td>
    <td style="padding:7px 0;border-bottom:.5px solid #E8E2D8;text-align:right;font-family:monospace;font-size:12px;color:${isOut ? '#B8471F' : '#8B6B3E'}">${i.stock} eftir</td>
  </tr>`;

  const section = (title, arr, isOut) => arr.length ? `
    <tr><td style="padding:20px 0 6px">
      <div style="font-family:monospace;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:${isOut ? '#B8471F' : '#8A8275'}">${title} (${arr.length})</div>
    </td></tr>
    <tr><td><table width="100%" cellpadding="0" cellspacing="0" style="font-size:13.5px">${arr.map(i => row(i, isOut)).join('')}</table></td></tr>` : '';
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#FAF7F2;border-radius:14px;padding:26px;">
  <tr><td>
    <div style="font-family:Georgia,serif;font-style:italic;font-size:21px;font-weight:500">${escapeHtml(machineName)}</div>
    <div style="font-family:monospace;font-size:11px;color:#8A8275;margin-top:3px">${escapeHtml(deviceCode)} · ${escapeHtml(statusBits.join(' · '))}</div>
  </td></tr>
  ${section('uppselt', out, true)}
  ${section('að klárast', low, false)}
  ${alerts.length ? `
  <tr><td style="padding:20px 0 6px"><div style="font-family:monospace;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#B8471F">athuga (${alerts.length})</div></td></tr>
  ${alerts.map(a => `<tr><td style="padding:6px 0;border-bottom:.5px solid #E8E2D8;font-size:13.5px">${escapeHtml(a.title)}</td></tr>`).join('')}` : ''}
  <tr><td style="padding:22px 0 0">
    <a href="${dashboardUrl}" style="display:inline-block;background:#1A1A1A;color:#FAF7F2;text-decoration:none;padding:11px 20px;border-radius:8px;font-size:14px">Opna vélina</a>
  </td></tr>
  <tr><td style="padding:18px 0 0;font-size:11px;color:#8A8275">${escapeHtml(operatorName)} — ${OPERATOR_BRAND}</td></tr>
</table></td></tr></table></body></html>`;
...
module.exports = { send, sendInvitation, sendComplaintToOperator, sendFridgeComplaintToOperator, sendComplaintAckToCustomer, sendComplaintReplyToCustomer, sendOperatorAlert, sendMachineDigest };
