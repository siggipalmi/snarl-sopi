...
async function syncOrders(deviceCode, days = 7) {
  ...
  (list || []).forEach(o => {
    ...
    // Order time: prefer per-item shipmentTime, else order-level fields.
    // Weimi reports these in China time (UTC+8) — see weimi.parseWeimiTime.
    const timeStr = first.shipmentTime || o.tradeStartTime || o.payEndTime || null;
    let ts = weimi.parseWeimiTime(timeStr);
    if (isNaN(ts)) ts = Date.now();

    // Upsert the line items first — one per detailVOList entry — for every order
    // Weimi returns, regardless of the summary cutoff, so the profit report gets
    // as much history as the order feed offers. Idempotent on (tradeNo,lineIndex).
    details.forEach((d, i) => {
      storage.upsertOrderItem({
        tradeNo, lineIndex: i, deviceCode,
        goodsId: d.goodsId || null,
        productName: weimi.fixMojibake(d.goodsName) || '',
        payAmount: typeof d.payAmount === 'number' ? d.payAmount : 0,
        shipmentStatus: d.shipmentStatus === 1 ? 1 : 0,
        createTime: ts,
      });
    });
