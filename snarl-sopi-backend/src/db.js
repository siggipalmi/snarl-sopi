...
function createInvitation({ email, name, role, operatorId, inviterId, machineAccess = 'all' }) {
  ...
  const invite = {
    token, email, name, role, operatorId,
    // The invitations table has inviterId NOT NULL. Self-service provisioning (the Zapier
    // operator-signup path) has no human inviter, so a literal null here throws a constraint
    // error that killed the request mid-flight (operator created, then 502). Use a sentinel.
    inviterId: inviterId || 'system',
    machineAccess,
    createdAt: Date.now(),
