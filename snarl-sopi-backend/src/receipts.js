...
// Cheap end-to-end key test: one tiny call. Returns { ok, model } on success,
// or { ok:false, status, error } describing exactly why it failed (no-key,
// auth, billing, etc.) so the operator can tell key-missing from key-invalid.
async function ping() {
  if (!API_KEY) return { ok: false, status: 'no-key', error: 'ANTHROPIC_API_KEY not set (check the variable name/casing in Railway)' };
  try {
    const resp = await fetch(API_URL, {
      method: 'POST',
    ...
    if (resp.ok) return { ok: true, model: MODEL };
    const t = await resp.text().catch(() => '');
    // Classify the common failures so the message is actionable.
    let kind = 'error';
    if (resp.status === 401) kind = 'auth (key rejected — wrong key or account)';
    else if (/credit|billing/i.test(t)) kind = 'billing (key valid but no credit)';
    else if (resp.status === 403) kind = 'permission (key lacks Messages API access)';
    return { ok: false, status: resp.status, kind, error: t.slice(0, 200) };
  } catch (e) {
    return { ok: false, status: 'network', error: e.message };
  }
}

module.exports = { extract, normalize, deriveNetUnit, parseModelJson, callVision, ping };
