...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS fridge_baskets (
    deviceCode      TEXT NOT NULL,
    cabinet         TEXT NOT NULL,            -- 'A' or 'B'
    basket          INTEGER NOT NULL,         -- tray id as the weight board addresses it
    serialLockNum   TEXT,                     -- opaque; passed through to the app
    productId       TEXT,                     -- NULL = empty/disabled basket
    priceIsk        INTEGER,                  -- override; else product salePriceIsk
    unitWeightG     INTEGER,                  -- override; else product weightGrams
    toleranceG      INTEGER,                  -- override; else default 15
    measurementFlag INTEGER,                  -- 0/1; default 1
  ...
  CREATE TABLE IF NOT EXISTS fridge_settlements (
    ...
    startedAt       TEXT,
    closedAt        TEXT,
    cabinetsOpened  TEXT,                     -- JSON array e.g. ["A","B"]
    outcome         TEXT,                     -- charged|nothing_taken|declined|sensor_fault|pending_offline
    totalIsk        INTEGER,                  -- app-reported
    recomputedIsk   INTEGER,                  -- backend-recomputed from planogram
    mismatch        INTEGER NOT NULL DEFAULT 0,
    nayaxRef        TEXT,
    anomalies       TEXT,                     -- JSON array, stored verbatim
  ...
  CREATE TABLE IF NOT EXISTS fridge_settlement_lines (
    ...
    deltaG          INTEGER,
    unitWeightG     INTEGER,
    quantity        INTEGER,                  -- app-reported
    recomputedQty   INTEGER,                  -- backend-recomputed
    priceIsk        INTEGER,
    lineIsk         INTEGER,
...
const storage = {
  ...
  saveFridgeSettlement: db.transaction((settlement, lines) => {
    ...
    stmts.deleteFridgeLines.run(settlement.orderId);   // replace lines wholesale on re-post
    for (const l of lines) stmts.insertFridgeLine.run(l);
  }),
  listFridgeSettlements(deviceCode, lim = 100) {
    const rows = stmts.listFridgeSettlements.all(deviceCode, { lim: Math.max(1, Math.min(lim, 500)) });
    return rows;
  },
