...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_fridge_lines_order ON fridge_settlement_lines(orderId);
  ...
  -- Supplier receipts/invoices ingested for product cost prices (feature A) and,
  -- later, expense push to Payday (feature B). We store full detail even where A
  -- ignores it, so B needs no re-extraction.
  CREATE TABLE IF NOT EXISTS receipts (
    id            TEXT PRIMARY KEY,
    operatorId    TEXT,
    supplier      TEXT,
    supplierKt    TEXT,
    date          TEXT,                     -- ISO yyyy-mm-dd from the receipt
    number        TEXT,                     -- invoice/receipt number
    currency      TEXT DEFAULT 'ISK',
    sourceType    TEXT,                     -- 'photo' | 'pdf'
    priceBasis    TEXT,                     -- 'net' | 'gross'
    netTotalIsk   INTEGER,
    vskTotalIsk   INTEGER,
    grossTotalIsk INTEGER,
  ...
  CREATE INDEX IF NOT EXISTS idx_receipts_op     ON receipts(operatorId);

  CREATE TABLE IF NOT EXISTS receipt_lines (
    id             TEXT PRIMARY KEY,
    receiptId      TEXT NOT NULL,
    idx            INTEGER,                 -- line order on the receipt
    description    TEXT,                    -- as printed (may be truncated)
    packSize       TEXT,                    -- raw, e.g. '18x330', '3 pk'
    unitsPerPack   INTEGER DEFAULT 1,       -- sellable singles per purchased pack
    qtyPacks       REAL,                    -- packs bought (Bonus stk; invoice line qty)
    barcode        TEXT,                    -- EAN if present
    priceBasis     TEXT,                    -- 'net' | 'gross' (usually receipt-level)
    pricePerPackIsk INTEGER,                -- price of one pack as printed
    lineTotalIsk   INTEGER,
    vatRate        INTEGER DEFAULT 11,      -- 11 | 24
    netUnitCostIsk INTEGER,                 -- DERIVED per-single-unit net; written to product on confirm
    matchedGoodsId TEXT,                    -- resolved in review (products.goodsId is TEXT)
    matchStatus    TEXT                     -- 'auto' | 'confirmed' | 'created' | 'unmatched'
  );
  CREATE INDEX IF NOT EXISTS idx_rlines_receipt ON receipt_lines(receiptId);

  -- Learned mappings: a receipt line's raw description -> the product an operator
  -- confirmed it as. Global (one shared catalog). On future extracts these give a
  -- confident auto-match regardless of how terse/odd the supplier's name is, so the
  -- first receipt teaches the matcher and later ones are mostly pre-matched.
  CREATE TABLE IF NOT EXISTS receipt_aliases (
    aliasKey   TEXT PRIMARY KEY,   -- normalized description (lowercased, collapsed)
    rawDesc    TEXT,               -- the description as last seen, for display
    goodsId    TEXT NOT NULL,
    hits       INTEGER DEFAULT 1,  -- how many times confirmed this way
    updatedAt  INTEGER
  );
  CREATE INDEX IF NOT EXISTS idx_aliases_goods ON receipt_aliases(goodsId);

  -- Draft products: a receipt line matched to a product that doesn't exist yet.
  -- The net cost parks here until the operator finishes the draft into a real
  -- catalog product, at which point the cost transfers onto it and the draft is
  -- deleted. Kept OUT of the products table so nothing (search, reports, catalog,
  -- Weimi sync) ever sees a half-real product.
  CREATE TABLE IF NOT EXISTS product_drafts (
    id             TEXT PRIMARY KEY,
    receiptId      TEXT,               -- the receipt this draft came from
    lineId         TEXT,               -- the receipt_line it was drafted from
    name           TEXT,               -- from the receipt description, operator-editable
    barcode        TEXT,
    packSize       TEXT,
    vatRate        INTEGER DEFAULT 11,
    costPriceNetIsk INTEGER,           -- the parked net cost, transferred on creation
    status         TEXT DEFAULT 'pending',  -- 'pending' | 'created'
    createdGoodsId TEXT,               -- set once turned into a real product
    createdAt      INTEGER,
    resolvedAt     INTEGER
  );
  CREATE INDEX IF NOT EXISTS idx_drafts_status  ON product_drafts(status);
  CREATE INDEX IF NOT EXISTS idx_drafts_receipt ON product_drafts(receiptId);
`);
...
// Net cost price (VSK-excluded), per single unit, whole ISK. Additive: the
// existing costPriceIsk stays gross and untouched. The profit report prefers
// this when present. Written from confirmed receipt lines (feature A).
ensureColumn('products', 'costPriceNetIsk', 'INTEGER');
...
const stmts = {
  ...
  reportItems: db.prepare(`
    SELECT oi.tradeNo, oi.deviceCode, oi.goodsId, oi.payAmount, oi.createTime,
           ...
           p.vatRate AS vatRate, p.costPriceIsk AS costPriceIsk, p.costPriceNetIsk AS costPriceNetIsk
    FROM order_items oi
  ...
  upsertFridgeBasket: db.prepare(`INSERT INTO fridge_baskets
      (deviceCode, cabinet, basket, serialLockNum, productId, priceIsk, unitWeightG, toleranceG, measurementFlag, enabled, updatedAt)
      VALUES (@deviceCode, @cabinet, @basket, @serialLockNum, @productId, @priceIsk, @unitWeightG, @toleranceG, @measurementFlag, @enabled, @updatedAt)
  ...
  // ── Fridge settlements ──
  getFridgeSettlement: db.prepare('SELECT * FROM fridge_settlements WHERE orderId = ?'),
  ...
  deleteFridgeLines: db.prepare('DELETE FROM fridge_settlement_lines WHERE orderId = ?'),
  insertFridgeLine: db.prepare(`INSERT INTO fridge_settlement_lines
      (orderId, cabinet, basket, productId, startWeightG, endWeightG, deltaG, unitWeightG, quantity, recomputedQty, priceIsk, lineIsk, lineMismatch)
      VALUES (@orderId, @cabinet, @basket, @productId, @startWeightG, @endWeightG, @deltaG, @unitWeightG, @quantity, @recomputedQty, @priceIsk, @lineIsk, @lineMismatch)`),
  listFridgeSettlements: db.prepare('SELECT * FROM fridge_settlements WHERE deviceCode = ? ORDER BY receivedAt DESC LIMIT @lim'),
  listFridgeLines: db.prepare('SELECT * FROM fridge_settlement_lines WHERE orderId = ?'),
  ...
  searchProducts: db.prepare(`SELECT goodsId, name, barcode, imgUrl, salePriceIsk,
                                     vatRate, costPriceIsk, costPriceNetIsk
                              FROM products
                              WHERE name LIKE @like OR barcode LIKE @pfx OR customCode LIKE @like
                              ORDER BY CASE WHEN name LIKE @pfx THEN 0 ELSE 1 END, name
                              LIMIT @lim`),

  // Feature A: write only the net cost, leaving gross costPriceIsk untouched.
  setProductNetCost: db.prepare(`UPDATE products SET costPriceNetIsk=@net, updatedAt=@now WHERE goodsId=@goodsId`),
  // Current cost for the sanity check (both columns + rate to normalize gross->net).
  productCost: db.prepare(`SELECT goodsId, name, vatRate, costPriceIsk, costPriceNetIsk FROM products WHERE goodsId=?`),

  insertReceipt: db.prepare(`INSERT INTO receipts
      (id, operatorId, supplier, supplierKt, date, number, currency, sourceType, priceBasis,
       netTotalIsk, vskTotalIsk, grossTotalIsk, status, createdBy, createdAt)
      VALUES (@id, @operatorId, @supplier, @supplierKt, @date, @number, 'ISK', @sourceType, @priceBasis,
       @netTotalIsk, @vskTotalIsk, @grossTotalIsk, 'draft', @createdBy, @createdAt)`),
  insertReceiptLine: db.prepare(`INSERT INTO receipt_lines
      (id, receiptId, idx, description, packSize, unitsPerPack, qtyPacks, barcode, priceBasis,
       pricePerPackIsk, lineTotalIsk, vatRate, netUnitCostIsk, matchedGoodsId, matchStatus)
      VALUES (@id, @receiptId, @idx, @description, @packSize, @unitsPerPack, @qtyPacks, @barcode, @priceBasis,
       @pricePerPackIsk, @lineTotalIsk, @vatRate, @netUnitCostIsk, @matchedGoodsId, @matchStatus)`),
  getReceipt:      db.prepare('SELECT * FROM receipts WHERE id=?'),
  getReceiptLines: db.prepare('SELECT * FROM receipt_lines WHERE receiptId=? ORDER BY idx'),
  listReceipts:    db.prepare(`SELECT r.*, (SELECT COUNT(*) FROM receipt_lines WHERE receiptId=r.id) AS lineCount
                               FROM receipts r WHERE (@op IS NULL OR operatorId=@op) ORDER BY createdAt DESC LIMIT @lim`),
  updateReceiptLine: db.prepare(`UPDATE receipt_lines SET
       unitsPerPack=@unitsPerPack, qtyPacks=@qtyPacks, priceBasis=@priceBasis,
       pricePerPackIsk=@pricePerPackIsk, vatRate=@vatRate, netUnitCostIsk=@netUnitCostIsk,
       matchedGoodsId=@matchedGoodsId, matchStatus=@matchStatus
       WHERE id=@id`),
  setReceiptConfirmed: db.prepare(`UPDATE receipts SET status='confirmed', confirmedAt=@now WHERE id=@id`),
  getAlias: db.prepare('SELECT aliasKey, rawDesc, goodsId, hits FROM receipt_aliases WHERE aliasKey=?'),
  upsertAlias: db.prepare(`INSERT INTO receipt_aliases (aliasKey, rawDesc, goodsId, hits, updatedAt)
      VALUES (@aliasKey, @rawDesc, @goodsId, 1, @now)
      ON CONFLICT(aliasKey) DO UPDATE SET goodsId=@goodsId, rawDesc=@rawDesc, hits=hits+1, updatedAt=@now`),
  insertDraft: db.prepare(`INSERT INTO product_drafts
      (id, receiptId, lineId, name, barcode, packSize, vatRate, costPriceNetIsk, status, createdAt)
      VALUES (@id, @receiptId, @lineId, @name, @barcode, @packSize, @vatRate, @costPriceNetIsk, 'pending', @createdAt)`),
  listPendingDrafts: db.prepare(`SELECT * FROM product_drafts WHERE status='pending' ORDER BY createdAt DESC LIMIT @lim`),
  getDraft:          db.prepare('SELECT * FROM product_drafts WHERE id=?'),
  markDraftCreated:  db.prepare(`UPDATE product_drafts SET status='created', createdGoodsId=@goodsId, resolvedAt=@now WHERE id=@id`),
  deleteDraft:       db.prepare('DELETE FROM product_drafts WHERE id=?'),
...
const storage = {
  ...
  // Delete a catalog row, but ONLY if nothing references its goodsId. Re-checks references
  // at delete time (never trusts a caller's stale view), so an orphan that quietly gained a
  // reference between audit and delete is left alone. Returns { deleted, reason }.
  deleteProductIfUnreferenced(goodsId) {
    const p = stmts.getProduct.get(goodsId);
    if (!p) return { deleted: false, reason: 'not found' };
    const refs = this.referenceCountsForGoods(goodsId);
  ...
  // ── Fridge settlements ──
  getFridgeSettlement(orderId) {
    const s = stmts.getFridgeSettlement.get(orderId);
    if (!s) return null;
    s.lines = stmts.listFridgeLines.all(orderId);
    return s;
  },
  saveFridgeSettlement: db.transaction((settlement, lines) => {
    stmts.upsertFridgeSettlement.run(settlement);
    stmts.deleteFridgeLines.run(settlement.orderId);
  ...
  // ─── Receipt cost ingestion (feature A) ─────────────────────────────────────
  // Write net cost for one product; leaves gross costPriceIsk untouched.
  setProductNetCost(goodsId, netIsk) {
    return stmts.setProductNetCost.run({ goodsId, net: Math.max(0, Math.round(netIsk)), now: Date.now() });
  },
  // Current cost for the sanity check. Returns { existingNet, source } where
  // existingNet is net-per-unit: prefer costPriceNetIsk; else derive from gross.
  currentNetCost(goodsId) {
    const p = stmts.productCost.get(goodsId);
    if (!p) return { found: false, existingNet: null, source: null };
    if (p.costPriceNetIsk != null) return { found: true, existingNet: p.costPriceNetIsk, source: 'net' };
    if (p.costPriceIsk != null) {
      const rate = (p.vatRate === 24) ? 24 : 11;
      return { found: true, existingNet: Math.round(p.costPriceIsk / (1 + rate / 100)), source: 'gross' };
    }
    return { found: true, existingNet: null, source: null };
  },
  createReceipt(r, lines) {
    const tx = db.transaction(() => {
      stmts.insertReceipt.run(r);
      lines.forEach(l => stmts.insertReceiptLine.run(l));
    });
    tx();
    return this.getReceiptFull(r.id);
  },
  getReceiptFull(id) {
    const receipt = stmts.getReceipt.get(id);
    if (!receipt) return null;
    receipt.lines = stmts.getReceiptLines.all(id);
    return receipt;
  },
  listReceipts(operatorId = null, limit = 50) {
    return stmts.listReceipts.all({ op: operatorId, lim: limit });
  },
  updateReceiptLine(line) { return stmts.updateReceiptLine.run(line); },
  confirmReceipt(id) { return stmts.setReceiptConfirmed.run({ id, now: Date.now() }); },

  // Normalize a supplier description into a stable alias key: lowercase, strip
  // punctuation, collapse whitespace. Pack specs stay in the key so
  // "pepsi max dós 18x330" and "pepsi max dós 12x330" can map to different sizes.
  aliasKeyFor(desc) {
    return String(desc || '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim().replace(/\s+/g, ' ');
  },
  // Look up a learned mapping for a description. Returns { goodsId, hits } or null.
  lookupAlias(desc) {
    const key = this.aliasKeyFor(desc);
    if (!key) return null;
    const row = stmts.getAlias.get(key);
    return row || null;
  },

  // Transactional confirm: write each resolved line's net cost to its product,
  // record the learned alias (description -> goodsId), update the line, and mark
  // the receipt confirmed — all or nothing.
  // writes: [{ line, goodsId, net, matchStatus }]
  confirmReceiptWrites: db.transaction(function (id, writes, drafts) {
    const done = [];
    const now = Date.now();
    for (const w of writes) {
      stmts.setProductNetCost.run({ goodsId: w.goodsId, net: Math.max(0, Math.round(w.net)), now });
      stmts.updateReceiptLine.run({
        id: w.line.id, unitsPerPack: w.line.unitsPerPack, qtyPacks: w.line.qtyPacks,
        priceBasis: w.line.priceBasis, pricePerPackIsk: w.line.pricePerPackIsk, vatRate: w.line.vatRate,
        netUnitCostIsk: Math.round(w.net), matchedGoodsId: w.goodsId, matchStatus: w.matchStatus || 'confirmed',
      });
      // Learn the mapping so this description auto-matches next time.
      const aliasKey = String(w.line.description || '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim().replace(/\s+/g, ' ');
      if (aliasKey) stmts.upsertAlias.run({ aliasKey, rawDesc: w.line.description, goodsId: w.goodsId, now });
      done.push({ goodsId: w.goodsId, netUnitCostIsk: Math.round(w.net) });
    }
    // Draft lines: park the cost in product_drafts for later creation. No product
    // exists yet, so nothing is written to the products table for these.
    const draftsMade = [];
    for (const d of (drafts || [])) {
      stmts.insertDraft.run({
        id: d.id, receiptId: id, lineId: d.lineId, name: d.name, barcode: d.barcode || null,
        packSize: d.packSize || null, vatRate: (Number(d.vatRate) === 24 ? 24 : 11),
        costPriceNetIsk: Math.max(0, Math.round(d.costPriceNetIsk)), createdAt: now,
      });
      stmts.updateReceiptLine.run({
        id: d.lineId, unitsPerPack: d.unitsPerPack || 1, qtyPacks: d.qtyPacks || 0,
        priceBasis: d.priceBasis || 'gross', pricePerPackIsk: d.pricePerPackIsk || null, vatRate: (Number(d.vatRate) === 24 ? 24 : 11),
        netUnitCostIsk: Math.max(0, Math.round(d.costPriceNetIsk)), matchedGoodsId: null, matchStatus: 'drafted',
      });
      draftsMade.push({ id: d.id, name: d.name });
    }
    stmts.setReceiptConfirmed.run({ id, now });
    return { written: done, drafts: draftsMade };
  }),

  // ─── Product drafts (deferred creation) ─────────────────────────────────────
  listPendingDrafts(limit = 100) { return stmts.listPendingDrafts.all({ lim: limit }); },
  getDraftById(id) { return stmts.getDraft.get(id); },
  // After a draft becomes a real product: transfer the parked net cost onto it
  // and mark the draft resolved.
  resolveDraft: db.transaction(function (draftId, goodsId) {
    const d = stmts.getDraft.get(draftId);
    if (!d) return null;
    if (d.costPriceNetIsk != null && goodsId) {
      stmts.setProductNetCost.run({ goodsId, net: Math.max(0, Math.round(d.costPriceNetIsk)), now: Date.now() });
    }
    stmts.markDraftCreated.run({ id: draftId, goodsId, now: Date.now() });
    return { draftId, goodsId, costTransferred: d.costPriceNetIsk };
  }),
  removeDraft(id) { return stmts.deleteDraft.run(id); },
  markDraftResolved(draftId, goodsId) { return stmts.markDraftCreated.run({ id: draftId, goodsId, now: Date.now() }); },
