...
const storage = {
  ...
  resolveIdleForMachine(deviceCode, opts) {
    ...
    const info = (gid, fb) => {
      ...
      const l = lp[gid] || {};
      const price = (p.salePriceIsk != null) ? p.salePriceIsk : (l.priceIsk || null);
      return {
        goodsId: gid,
    ...
    return { rotationSeconds: 6, cards };
  },
  ...
  priceCartForMachine(deviceCode, items) {
    ...
    const r0 = (n) => Math.round(n);
    const lp = this.layoutProductsForDevice(deviceCode) || {};
    const priceOf = (gid) => { const p = stmts.getProduct.get(gid) || {}; const l = lp[gid] || {}; const v = (p.salePriceIsk != null) ? p.salePriceIsk : (l.priceIsk != null ? l.priceIsk : null); return v != null ? v : 0; };
  ...
  offersForMachine(deviceCode) {
    ...
    for (const gid of Object.keys(lp)) { const ex = this._expiryForProduct(deviceCode, gid, expiryDeals, today); if (ex) setIf(gid, { goodsId: gid, kind: 'expiry', value: ex.percent }, 0); }
    for (const d of markdownDeals) { const cfg = d.config || {}; for (const gid of Object.keys(lp)) if (this._dealCoversProduct(d.appliesTo, gid)) setIf(gid, { goodsId: gid, kind: cfg.mode || 'percent', value: cfg.value || 0 }, 1); }
    for (const d of multibuyDeals) { for (const gid of Object.keys(lp)) if (this._dealCoversProduct(d.appliesTo, gid)) setIf(gid, { goodsId: gid, kind: 'multibuy' }, 2); }
    for (const d of comboDeals) { for (const g of (d.config && d.config.groups || [])) for (const p of (g.products || [])) setIf(String(p.goodsId), { goodsId: String(p.goodsId), kind: 'combo' }, 3); }
