...
const storage = {
  ...
  stockForProduct(goodsId, machines) {
    if (Array.isArray(machines) && machines.length) {
      const ph = machines.map(() => '?').join(',');
      const row = db.prepare(`SELECT COALESCE(SUM(stock),0) AS n FROM slot_stock WHERE goodsId = ? AND deviceCode IN (${ph})`).get(goodsId, ...machines);
      return row ? row.n : 0;
    }
    const row = stmts.sumSlotStock.get(goodsId);
    return row ? row.n : 0;
  },
  ...
  resolveIdleForMachine(deviceCode, opts) {
    ...
    const stockRows = stmts.listSlotStockForDevice.all(deviceCode) || [];
    const stockMap = {}; for (const r of stockRows) stockMap[r.goodsId] = r;
    const inStock = (gid) => stockMap[gid] && stockMap[gid].stock > 0;
    const info = (gid, fb) => {
      const p = stmts.getProduct.get(gid) || {};
      return {
        goodsId: gid,
        name: p.name || fb || (stockMap[gid] && stockMap[gid].productName) || gid,
        imgUrl: p.imgUrl || null,
        priceIsk: p.salePriceIsk != null ? p.salePriceIsk : null,
    ...
    for (const d of deals) {
      ...
      else if (at.kind === 'all') ids = stockRows.map(r => r.goodsId);
      else if (at.kind === 'all_perishable') ids = stockRows.map(r => r.goodsId).filter(gid => { const p = stmts.getProduct.get(gid); return p && p.perishable; });
      ...
      else items.sort((a, b) => ((stockMap[a.goodsId] ? stockMap[a.goodsId].stock : 0) - (stockMap[b.goodsId] ? stockMap[b.goodsId].stock : 0)) || String(a.name).localeCompare(String(b.name)));
