...
const storage = {
  ...
  // update the line, and mark the receipt confirmed — all or nothing.
  // writes: [{ line, goodsId, net, matchStatus }]
  confirmReceiptWrites: db.transaction(function (id, writes) {
    const done = [];
    for (const w of writes) {
      stmts.setProductNetCost.run({ goodsId: w.goodsId, net: Math.max(0, Math.round(w.net)), now: Date.now() });
    ...
    stmts.setReceiptConfirmed.run({ id, now: Date.now() });
    return done;
  }),
